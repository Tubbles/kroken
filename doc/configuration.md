...
kroken reads TOML files only. Run `kroken config` (optionally with `--file path` and `--profile name`) to print the files that were found and the effective values, defaults included.
...
| # | Location | Purpose |
|---|----------|---------|
| 1 | `<dir>/kroken/config.toml` for each `<dir>` in `$XDG_CONFIG_DIRS` (default `/etc/xdg`) | Machine-wide defaults. Earlier entries in the variable win over later ones, per the XDG specification. |
| 2 | `$XDG_CONFIG_HOME/kroken/config.toml` (default `~/.config/kroken/config.toml`) | Your defaults. |
| 3 | `$XDG_CONFIG_HOME/kroken/config.d/*.toml`, sorted by file name | Your defaults, split across files. Keep `10-shared.toml` in your dotfiles repository and `90-local.toml` out of it. |
| 4 | `.kroken` in the target file's directory and every parent, applied from the filesystem root downwards | Project settings. The file closest to the target wins. With no `--file`, discovery starts at the working directory. |
| 5 | Command line flags (`--profile`, `--model`) | This run only. |

## Merge rules

Tables merge recursively. Any other value, arrays included, replaces the value from the layer below wholesale. A `tools = ["Read"]` in a `.kroken` file therefore replaces the whole list, it does not append.

## Profiles

A profile is a named subtree under `[profiles.<name>]` holding any of the keys below. After all layers are merged, the `profile` key names the profile to apply, and that subtree is merged on top of everything one more time. `--profile` on the command line beats the key. Naming a profile that no file defines is an error.
...
```toml
profile = ""                        # name of the [profiles.*] subtree to apply
...
max_budget_usd = 0.0                # --max-budget-usd when greater than zero
add_git_root = true                 # pass --add-dir <git root> so read-only tools can reach the whole repository

[claude.env]                        # environment variables set for the claude process; "~/" is expanded
...
[profiles.<name>]                   # any of the keys above, applied when profile = "<name>"
```
...
Both `prompt.template` and, if you want them there, `prompt.system` are used as given; only `prompt.template` is rendered. Placeholders:
