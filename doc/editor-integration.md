...
```sh
kroken complete --file <path> [--start LINE[:COLUMN]] [--end LINE[:COLUMN]] [--selection-file <path>] [--profile <name>] [--backend <name>] [--model <name>]
```
...
- `--file` is the path of the buffer the selection came from. Its directory becomes the working directory of the backend, which is what makes its instruction files above the file load (`CLAUDE.md` for Claude Code, `AGENTS.md` for Codex). `--start` and `--end` are optional; they inform the prompt and, when nothing is piped in, pick the lines to read from the file. Positions are 1-based and the end is inclusive.
...
- stderr carries human-readable status: a final `kroken: done in 4.2 s via claude, 3 turns, $0.0421` line, the log directory, and any error. Show it in the status bar or ignore it.
- Exit codes: `0` success, `1` the backend ran and reported an error (stderr says why), `2` bad arguments or configuration, `3` the backend could not be started or returned something unparseable. Anything but `0` means stdout holds no replacement.
