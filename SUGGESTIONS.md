...
- **`.kroken` as a directory.** Large projects might want `.kroken/*.sjson` like `config.d/`. `discover` in `src/config/discover.odin` would need one more branch. Left out for simplicity.
...
- **Report the silent unterminated string upstream.** In Odin `dev-2026-09`, `core:encoding/json` in SJSON and JSON5 mode returns an empty string for `a = "open` followed by a newline instead of an error, because `advance_token` discards the tokenizer's `String_Not_Terminated`. `find_unterminated_string` in `src/config/sjson.odin` works around it; an upstream fix would let that scan go.
- **Codex session resume.** `codex exec resume <SESSION_ID>` exists, and the backend already records the thread id in `Result.session_id`. A follow-up flag in `run_complete` could pass it on for "refine this" round trips, once persist_session is on.
- **pandoc as a second man converter.** `scripts/build-man.sh` only knows go-md2man. pandoc would accept the same Markdown except for the title line (`# name section date source manual` versus pandoc's `%` metadata block); a small branch in the script could cover machines that have pandoc and no Go.
- **Man page for `kroken help` too.** `kroken help` prints raw Markdown, which reads fine but is not paged. Piping through `$PAGER` when stdout is a terminal would match `man`.
